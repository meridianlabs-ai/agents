...
| `inspect-harbor-like` | inspect_harbor's `claude-setup` (Python 3.12, `uv sync`, `default-groups = ["dev", "doc"]`) | `uv venv --python 3.12 && uv sync` | Python 3.12; `pytest` (dev) and `markdown` (doc) importable; lock unchanged |
...
Python projects, `COREPACK_ENABLE_DOWNLOAD_PROMPT=0 corepack pnpm install
--lockfile-only` in the Node one.
