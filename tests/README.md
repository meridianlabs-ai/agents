...
- `test_engine_job_isolation.py` — one untrusted job per engine (Claude
  ...
  hold) and its `reset-home` step (kills, then the home script with the
  `bin` input; refuses while `pkill` keeps finding codex processes), and
  every codex job's reset step passes the provisioning step's `bin` output.
